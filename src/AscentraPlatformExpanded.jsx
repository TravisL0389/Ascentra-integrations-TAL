...
import { X, Check, ArrowRight, Sparkles, Code2, Workflow, MessageSquare,
         ...
         FileText, Mail, Rocket, Search, TerminalSquare, Users,
         Zap, GitBranch, Timer, Webhook, Database, Send, Filter,
         PlusCircle, Save, Activity, CircleCheck, CircleAlert } from 'lucide-react';
...
const AGENTS = [
  ...
  { id:'pulse',  name:'Pulse',  specialty:'Automation & Orchestration',       tagline:'The orchestration core where atomic actions snap into living workflows.',     color:'#0dab6f', glow:'rgba(13,171,111,0.4)',  orbit:0, angleOffset:240, Icon:Workflow,
    capabilities:['Atomic workflow builder with branching logic','Multi-agent orchestration across Veyra, Forge, Nexus','Conditional routing, delays & error recovery','Live execution traces & versioned deploys'],
    prompts:['Build a deal-closed pipeline that briefs the exec and schedules onboarding.','Route support tickets by sentiment, then auto-draft a reply.','Run the weekly metrics digest every Monday at 07:00.'] },
...
const AgentCharacter = ({ agent, size = 180 }) => {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" style={{ overflow:'visible' }}>
      ...
      {id==='pulse'  && <>
        {/* Orchestration halo — 3 satellite modules bonded to the core, pulsing in sequence */}
        {/* Bond lines (drawn first, behind satellites) */}
        <path d="M 100 45 L 58 30"  stroke={color} strokeWidth="1.2" fill="none" opacity="0.45" style={{ animation:'bond-glow-a 3.6s ease-in-out infinite' }}/>
        <path d="M 100 45 L 100 20" stroke={color} strokeWidth="1.2" fill="none" opacity="0.45" style={{ animation:'bond-glow-b 3.6s ease-in-out infinite' }}/>
        <path d="M 100 45 L 142 30" stroke={color} strokeWidth="1.2" fill="none" opacity="0.45" style={{ animation:'bond-glow-c 3.6s ease-in-out infinite' }}/>
        {/* Soft glow behind the crown halo */}
        <ellipse cx="100" cy="28" rx="52" ry="14" fill={color} opacity="0.14" filter="blur(6px)"/>
        {/* Satellite module 1 — ACTION (round) */}
        <g style={{ transformOrigin:'58px 30px', animation:'sat-breathe-a 3.6s ease-in-out infinite' }}>
          <circle cx="58" cy="30" r="9" fill={`url(#b-${id})`} stroke={shadeColor(color,-25)} strokeWidth="1"/>
          <circle cx="55.5" cy="27.5" r="2.5" fill={`url(#sh-${id})`} opacity="0.6"/>
          <circle cx="56" cy="30" r="0.9" fill="#0a1218"/>
          <circle cx="60" cy="30" r="0.9" fill="#0a1218"/>
        </g>
        {/* Satellite module 2 — CONDITION (diamond) */}
        <g style={{ transformOrigin:'100px 20px', animation:'sat-breathe-b 3.6s ease-in-out infinite' }}>
          <path d="M 100 11 L 109 20 L 100 29 L 91 20 Z" fill={`url(#b-${id})`} stroke={shadeColor(color,-25)} strokeWidth="1"/>
          <circle cx="98" cy="20" r="0.9" fill="#0a1218"/>
          <circle cx="102" cy="20" r="0.9" fill="#0a1218"/>
        </g>
        {/* Satellite module 3 — ROUTER (hex) */}
        <g style={{ transformOrigin:'142px 30px', animation:'sat-breathe-c 3.6s ease-in-out infinite' }}>
          <path d="M 142 21 L 149 25 L 149 35 L 142 39 L 135 35 L 135 25 Z" fill={`url(#b-${id})`} stroke={shadeColor(color,-25)} strokeWidth="1"/>
          <circle cx="140" cy="30" r="0.9" fill="#0a1218"/>
          <circle cx="144" cy="30" r="0.9" fill="#0a1218"/>
        </g>
      </>}
...
// ─────────────────────────────────────────────────────────────────────────────
// AUTOMATION ATOM GRID BUILDER
// ─────────────────────────────────────────────────────────────────────────────
const ATOM_TYPES = {
  trigger:     { label:'Trigger',     color:'#06B6D4', Icon:Webhook,        desc:'Start a workflow from an event' },
  condition:   { label:'Condition',   color:'#F59E0B', Icon:GitBranch,      desc:'Branch the flow on logic' },
  action:      { label:'Action',      color:'#0dab6f', Icon:Zap,            desc:'Do something in a connected app' },
  router:      { label:'Router',      color:'#A855F7', Icon:Filter,         desc:'Split into parallel paths' },
  delay:       { label:'Delay',       color:'#6366F1', Icon:Timer,          desc:'Pause for time or signal' },
  integration: { label:'Integration', color:'#3B82F6', Icon:Database,       desc:'Read or write to a system of record' },
  output:      { label:'Output',      color:'#10B981', Icon:Send,           desc:'Emit, log, or close the workflow' },
};

// Workflow library — 3 saved pipelines the user can switch between
const WORKFLOWS = [
  {
    id: 'wf_deals',
    name: 'Deal-closed → onboarding pipeline',
    status: 'deployed',
    lastRun: '2m ago',
    runs: 1247,
    successRate: 98.4,
    runSequence: ['n1', 'n2', 'n3', 'n4', 'n6'],
    errorAt: 'n3', // which node fails in error-demo mode
    nodes: [
      { id:'n1', type:'trigger',     label:'Deal Closed',          subtitle:'Salesforce · Opportunity.Won', x:40,  y:220 },
      { id:'n2', type:'condition',   label:'Check Deal Value',     subtitle:'Amount > $10k',                x:260, y:220 },
      { id:'n3', type:'action',      label:'Notify Executive',     subtitle:'Slack · #c-suite',             x:500, y:90  },
      { id:'n4', type:'action',      label:'Schedule Onboarding',  subtitle:'Calendar · kickoff call',      x:500, y:200 },
      { id:'n5', type:'action',      label:'Auto Follow-up',       subtitle:'Email · nurture drip',         x:500, y:350 },
      { id:'n6', type:'integration', label:'Log to Warehouse',     subtitle:'Snowflake · events table',     x:740, y:220 },
    ],
    edges: [
      { from:'n1', to:'n2' },
    ...
    history: [
      { id:'r1', time:'2m ago',   status:'success', duration:'1.4s', note:'All 5 atoms fired' },
      { id:'r2', time:'18m ago',  status:'success', duration:'1.2s', note:'All 5 atoms fired' },
      { id:'r3', time:'1h ago',   status:'failed',  duration:'0.8s', note:'Slack API rate limit' },
      { id:'r4', time:'3h ago',   status:'success', duration:'1.3s', note:'All 5 atoms fired' },
      { id:'r5', time:'5h ago',   status:'success', duration:'1.5s', note:'All 5 atoms fired' },
    ],
  },
  {
    id: 'wf_support',
    ...
    runs: 3402,
    successRate: 99.1,
    runSequence: ['s1', 's2', 's3', 's4'],
    errorAt: 's3',
    nodes: [
      { id:'s1', type:'trigger',     label:'Ticket Created',      subtitle:'Zendesk · ticket.new',        x:40,  y:220 },
      { id:'s2', type:'condition',   label:'Sentiment Analysis',  subtitle:'Axiom · NLP classifier',      x:260, y:220 },
      { id:'s3', type:'router',      label:'Route by Priority',   subtitle:'urgent · normal · backlog',   x:500, y:220 },
      { id:'s4', type:'action',      label:'Auto-draft Reply',    subtitle:'Echo · brand-tuned voice',    x:740, y:220 },
    ],
    edges: [
      { from:'s1', to:'s2' },
      { from:'s2', to:'s3', label:'negative' },
      { from:'s3', to:'s4', label:'urgent' },
    ],
    history: [
      { id:'r1', time:'12m ago', status:'success', duration:'0.9s', note:'Routed to #urgent' },
      { id:'r2', time:'22m ago', status:'success', duration:'0.7s', note:'Routed to #normal' },
      { id:'r3', time:'34m ago', status:'success', duration:'0.8s', note:'Routed to #backlog' },
    ],
  },
  {
    id: 'wf_digest',
    ...
    runs: 24,
    successRate: 100,
    runSequence: ['d1', 'd2', 'd3', 'd4'],
    errorAt: 'd3',
    nodes: [
      { id:'d1', type:'trigger',     label:'Weekly Schedule',     subtitle:'Monday · 07:00 UTC',          x:40,  y:220 },
      { id:'d2', type:'integration', label:'Pull Analytics',      subtitle:'Mixpanel · last 7 days',      x:260, y:220 },
      { id:'d3', type:'action',      label:'Generate Summary',    subtitle:'Lumen · insights synth',      x:500, y:220 },
      { id:'d4', type:'output',      label:'Email Leadership',    subtitle:'leadership@company.com',      x:740, y:220 },
    ],
    edges: [
      { from:'d1', to:'d2' },
      { from:'d2', to:'d3' },
    ...
    history: [
      { id:'r1', time:'2d ago',  status:'success', duration:'4.2s', note:'Digest sent to 7 recipients' },
      { id:'r2', time:'9d ago',  status:'success', duration:'3.8s', note:'Digest sent to 7 recipients' },
      { id:'r3', time:'16d ago', status:'success', duration:'4.1s', note:'Digest sent to 7 recipients' },
    ],
  },
];

// A branded "atom icon" — small sphere with tiny face, used in node cards & toolbox
const AtomIcon = ({ color, size = 28, running = false }) => {
  ...
  return (
    <div style={{
      ...
      borderRadius:'50%',
      background:`radial-gradient(circle at 30% 28%, ${light}, ${color} 58%, ${dark})`,
      boxShadow:`0 0 ${running?14:6}px ${color}77, inset -2px -3px 5px rgba(0,0,0,0.4), inset 2px 3px 5px rgba(255,255,255,0.2)`,
      transition:'box-shadow 0.35s',
    }}>
      {/* shine */}
      <div style={{ position:'absolute', top:'18%', left:'22%', width:'30%', height:'22%', borderRadius:'50%', background:'rgba(255,255,255,0.5)', filter:'blur(1px)' }}/>
      {/* tiny eyes — mascot vibe */}
      <div style={{ position:'absolute', top:'54%', left:'32%', width:2, height:2.4, borderRadius:'50%', background:'#0a1218' }}/>
      <div style={{ position:'absolute', top:'54%', left:'60%', width:2, height:2.4, borderRadius:'50%', background:'#0a1218' }}/>
    </div>
  );
};

// Connection bond — curved SVG path between two node anchors
const Bond = ({ from, to, label, running, success, error }) => {
  const fromX = from.x + 188; // right port of source card
  const fromY = from.y + 34;
  const toX = to.x;            // left port of target card
  const toY = to.y + 34;
  const dx = Math.abs(toX - fromX);
  const c1X = fromX + Math.max(40, dx * 0.5);
  const c2X = toX   - Math.max(40, dx * 0.5);
  const path = `M ${fromX} ${fromY} C ${c1X} ${fromY}, ${c2X} ${toY}, ${toX} ${toY}`;
  const midX = (fromX + toX) / 2;
  const midY = (fromY + toY) / 2;

  const stroke  = error ? '#ef4444' : success ? '#10B981' : running ? '#00C9A7' : 'rgba(148, 197, 184, 0.55)';
  const glowCol = error ? 'rgba(239,68,68,0.8)' : success ? 'rgba(16,185,129,0.8)' : 'rgba(0,201,167,0.8)';

  return (
    <g>
      {/* Glow under-layer when active/error */}
      {(running || success || error) && (
        <path d={path} stroke={glowCol} strokeWidth="5" fill="none" opacity={error?0.5:0.35} style={{ filter:'blur(3px)' }}/>
      )}
      {/* Main bond line */}
      <path d={path} stroke={stroke} strokeWidth={running||error?2.2:1.6} fill="none"
            strokeDasharray={running?'6 5':error?'4 3':'0'}
            style={{ animation: running?'dash-march 1.1s linear infinite':'none' }}/>
      {/* Traveling signal dot when running */}
      {running && !error && (
        <circle r="3.2" fill="#d4f8ef" style={{
          offsetPath:`path('${path}')`,
          animation:'signal-travel 1.6s ease-in-out infinite',
          filter:'drop-shadow(0 0 4px rgba(0,201,167,0.9))',
        }}/>
      )}
      {/* Branch label */}
      {label && (
        <g>
          <rect x={midX - 14} y={midY - 10} width="28" height="16" rx="8" fill="rgba(7,13,22,0.92)" stroke={stroke} strokeWidth="1"/>
          <text x={midX} y={midY + 2} textAnchor="middle" fontFamily="JetBrains Mono, monospace" fontSize="8" fontWeight="600" fill={stroke} letterSpacing="0.1em" style={{ textTransform:'uppercase' }}>{label}</text>
        </g>
      )}
    </g>
  );
...
// Canvas node — the atom-themed workflow step card
const BuilderNode = ({ node, selected, running, success, error, onClick, onMouseDown }) => {
  const type = ATOM_TYPES[node.type];
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: selected ? 10 : running ? 5 : 1,
      }}>
      {/* Card */}
      <div style={{
        position:'relative', width:'100%', height:'100%',
        borderRadius:12,
        background:`linear-gradient(150deg, ${accent}18, rgba(11,17,30,0.92) 65%)`,
        border:`1px solid ${error ? '#ef4444' : selected ? accent : running ? accent+'aa' : accent+'44'}`,
        boxShadow: error
          ? `0 0 0 2px rgba(239,68,68,0.25), 0 10px 26px rgba(239,68,68,0.35)`
          : selected
          ? `0 0 0 2px ${accent}33, 0 12px 28px ${accent}33`
          : running
          ? `0 0 0 2px ${accent}33, 0 8px 22px ${accent}40`
          : `0 4px 14px rgba(0,0,0,0.3)`,
        display:'flex', alignItems:'center', gap:10,
        padding:'8px 12px',
        animation: error ? 'error-shake 0.5s ease-in-out' : running ? 'node-breathe 1.8s ease-in-out infinite' : 'none',
      }}>
        {/* Running ring */}
        {running && !error && (
          <div style={{ position:'absolute', inset:-3, borderRadius:14, border:`1.5px dashed ${accent}`, animation:'orbit-spin 4s linear infinite', opacity:0.7, pointerEvents:'none' }}/>
        )}
        {/* Error ring */}
        {error && (
          <div style={{ position:'absolute', inset:-3, borderRadius:14, border:`1.5px dashed #ef4444`, opacity:0.6, pointerEvents:'none' }}/>
        )}
        {/* Atom avatar */}
        <AtomIcon color={accent} size={30} running={running}/>
        {/* Labels */}
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:5 }}>
            <TypeIcon size={10} color={accent} style={{ opacity:0.9 }}/>
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8, letterSpacing:'0.2em', color:accent, textTransform:'uppercase', fontWeight:600 }}>{type.label}</span>
          </div>
          <div style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:12.5, fontWeight:700, color:'#fff', lineHeight:1.15, marginTop:2, letterSpacing:'-0.01em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{node.label}</div>
          <div style={{ fontFamily:'Manrope, sans-serif', fontSize:10, color:'rgba(255,255,255,0.5)', marginTop:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{node.subtitle}</div>
        </div>
        {/* Status badge */}
        {success && !error && (
          <div style={{ position:'absolute', top:-6, right:-6, width:18, height:18, borderRadius:'50%', background:'#10B981', border:'2px solid #070d16', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <Check size={10} color="#fff" strokeWidth={3}/>
          </div>
        )}
        {error && (
          <div style={{ position:'absolute', top:-6, right:-6, width:18, height:18, borderRadius:'50%', background:'#ef4444', border:'2px solid #070d16', display:'flex', alignItems:'center', justifyContent:'center', animation:'pulse-glow 1.2s ease-in-out infinite' }}>
            <X size={10} color="#fff" strokeWidth={3}/>
          </div>
        )}
        {/* Input port */}
        <div style={{ position:'absolute', left:-4, top:'50%', transform:'translateY(-50%)', width:8, height:8, borderRadius:'50%', background:accent, border:'2px solid #070d16', boxShadow:`0 0 0 1px ${accent}44` }}/>
        {/* Output port */}
        <div style={{ position:'absolute', right:-4, top:'50%', transform:'translateY(-50%)', width:8, height:8, borderRadius:'50%', background:accent, border:'2px solid #070d16', boxShadow:`0 0 0 1px ${accent}44` }}/>
      </div>
    </div>
  );
};

const BuilderPage = ({ onBack }) => {
  // Which workflow is open
  const [wfId, setWfId] = useState(WORKFLOWS[0].id);
  const sourceWf = WORKFLOWS.find(w => w.id === wfId);

  // Local editable copy of nodes/edges for the current workflow (supports DnD & repositioning)
  const [nodes, setNodes] = useState(sourceWf.nodes);
  const [edges, setEdges] = useState(sourceWf.edges);
  const [selectedId, setSelectedId] = useState(sourceWf.nodes[0]?.id);
  const [inspectorTab, setInspectorTab] = useState('Configure');

  // Reload local state whenever the user switches workflows
  useEffect(() => {
    const w = WORKFLOWS.find(x => x.id === wfId);
    setNodes(w.nodes);
    setEdges(w.edges);
    setSelectedId(w.nodes[0]?.id);
    setRunState({ mode:'idle', active:null, completed:[] });
  }, [wfId]);

  // Run state
  const [runState, setRunState] = useState({ mode:'idle', active:null, completed:[] });
  const [errorMode, setErrorMode] = useState(false);
  const [switcherOpen, setSwitcherOpen] = useState(false);
  const [dropPreview, setDropPreview] = useState(null); // { x, y } for ghost preview while dragging

  const canvasRef = useRef(null);
  const dragStateRef = useRef({ nodeId: null, offsetX: 0, offsetY: 0 }); // for repositioning existing nodes
  const timersRef = useRef([]);

  const selected = nodes.find(n => n.id === selectedId);
  const nodeById = useMemo(() => Object.fromEntries(nodes.map(n => [n.id, n])), [nodes]);

  // Cleanup any pending timers when unmounting or switching workflow
  ...
  const testRun = () => {
    clearRunTimers();
    const seq = sourceWf.runSequence;
    const errorAt = errorMode ? sourceWf.errorAt : null;

    setRunState({ mode:'running', active:null, completed:[] });

    for (let i = 0; i < seq.length; i++) {
      const id = seq[i];
      const hitsError = id === errorAt;

      // Activate this node
      timersRef.current.push(setTimeout(() => {
        setRunState(s => ({ mode:'running', active:id, completed:s.completed }));
      }, i * 900));

      if (hitsError) {
        // Halt at this node with error — no further progression
        timersRef.current.push(setTimeout(() => {
          setRunState({ mode:'failed', active:null, completed:seq.slice(0, i), failedAt: id });
        }, i * 900 + 750));
        return; // stop scheduling later steps
      }

      // Mark as complete after a beat
      timersRef.current.push(setTimeout(() => {
        setRunState(s => ({ mode:'running', active:null, completed:[...s.completed, id] }));
      }, i * 900 + 750));
    }
    // Finish success
    timersRef.current.push(setTimeout(() => {
      setRunState({ mode:'success', active:null, completed:seq });
    }, seq.length * 900 + 100));
  };
  ...
  // Edge state helpers
  const isEdgeActive = (e) => runState.mode === 'running' && runState.active === e.to;
  const isEdgeComplete = (e) =>
    runState.completed.includes(e.from) && runState.completed.includes(e.to);
  const isEdgeError = (e) =>
    runState.mode === 'failed' && runState.failedAt === e.to;

  const statusLabel = runState.mode === 'running' ? 'Running'
    : runState.mode === 'success' ? 'Success'
    : runState.mode === 'failed'  ? 'Failed'
    : 'Idle';
  const statusColor = runState.mode === 'running' ? '#00C9A7'
    : runState.mode === 'success' ? '#10B981'
    : runState.mode === 'failed'  ? '#ef4444'
    : 'rgba(255,255,255,0.45)';

  // ─── DROP FROM TOOLBOX → CANVAS (HTML5 DnD) ─────────────────────
  const handleCanvasDragOver = (e) => {
    if (e.dataTransfer.types.includes('application/atom-type')) {
      e.preventDefault();
      e.dataTransfer.dropEffect = 'copy';
      const rect = canvasRef.current.getBoundingClientRect();
      setDropPreview({ x: e.clientX - rect.left - 94, y: e.clientY - rect.top - 34 });
    }
  };
  const handleCanvasDragLeave = () => setDropPreview(null);
  const handleCanvasDrop = (e) => {
    e.preventDefault();
    const type = e.dataTransfer.getData('application/atom-type');
    if (!type || !ATOM_TYPES[type]) return;
    const rect = canvasRef.current.getBoundingClientRect();
    const newId = `u${Date.now().toString(36)}`;
    const newNode = {
      id: newId,
      type,
      label: `New ${ATOM_TYPES[type].label}`,
      subtitle: 'Click to configure',
      x: Math.max(10, e.clientX - rect.left - 94),
      y: Math.max(10, e.clientY - rect.top - 34),
    };
  ...
  // ─── REPOSITION EXISTING NODES (mousedown drag) ─────────────────
  const handleNodeMouseDown = useCallback((nodeId) => (e) => {
    ...
    const onMove = (ev) => {
      ...
      setNodes(prev => prev.map(n => n.id === s.nodeId ? { ...n, x: Math.max(0, nx), y: Math.max(0, ny) } : n));
    };
    const onUp = () => {
  ...
  // ─── RENDER ─────────────────────────────────────────────────────
  return (
    <main style={{ minHeight:'100vh', padding:'90px 32px 40px', position:'relative', overflow:'hidden' }}>
      {/* Atmosphere */}
      <div style={{ position:'absolute', inset:0, background:`radial-gradient(circle at 18% 10%, rgba(13,171,111,0.12), transparent 38%), radial-gradient(circle at 82% 18%, rgba(0,201,167,0.08), transparent 34%), ${BG}` }}/>
      <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(circle, rgba(255,255,255,0.04) 1px, transparent 1px)', backgroundSize:'34px 34px', opacity:0.5 }}/>

      <div style={{ position:'relative', maxWidth:1440, margin:'0 auto' }}>
        {/* Top strip: back / title + switcher / status / actions */}
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:18, marginBottom:14, flexWrap:'wrap' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14, minWidth:0, flex:1 }}>
            <button onClick={onBack} style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 12px', borderRadius:9, border:`1px solid ${BORD}`, background:SURF, color:'rgba(255,255,255,0.72)', fontFamily:'Bricolage Grotesque, sans-serif', fontWeight:700, fontSize:12, cursor:'pointer', flexShrink:0 }}>
              <ArrowLeft size={13}/> Back
            </button>
            {/* Workflow switcher */}
            <div style={{ position:'relative', minWidth:0 }}>
              <button onClick={() => setSwitcherOpen(v => !v)} style={{ display:'flex', alignItems:'center', gap:10, padding:'6px 10px 6px 6px', borderRadius:10, border:`1px solid ${switcherOpen ? ACCENT+'66' : BORD}`, background:switcherOpen ? `rgba(0,201,167,0.08)` : 'rgba(10,16,28,0.6)', cursor:'pointer', minWidth:0, maxWidth:500 }}>
                <div style={{ width:32, height:32, borderRadius:8, background:`linear-gradient(135deg, rgba(0,201,167,0.18), rgba(13,171,111,0.1))`, border:`1px solid rgba(0,201,167,0.25)`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <Workflow size={14} color={ACCENT}/>
                </div>
                <div style={{ textAlign:'left', minWidth:0, flex:1 }}>
                  <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.24em', color:'#0dab6f', textTransform:'uppercase', marginBottom:2 }}>Pulse · Workflow</div>
                  <div style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:15, fontWeight:800, letterSpacing:'-0.015em', color:'#fff', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{sourceWf.name}</div>
                </div>
                <ChevronDown size={14} color="rgba(255,255,255,0.5)" style={{ transform: switcherOpen ? 'rotate(180deg)' : 'rotate(0)', transition:'transform 0.2s', flexShrink:0 }}/>
              </button>
              {switcherOpen && (
                <>
                  {/* Click-outside catcher */}
                  <div onClick={() => setSwitcherOpen(false)} style={{ position:'fixed', inset:0, zIndex:40 }}/>
                  <div style={{ position:'absolute', top:'calc(100% + 6px)', left:0, zIndex:50, width:380, borderRadius:12, background:'rgba(10,16,28,0.96)', border:`1px solid ${BORD}`, boxShadow:'0 18px 42px rgba(0,0,0,0.45)', backdropFilter:'blur(16px)', overflow:'hidden' }}>
                    <div style={{ padding:'10px 14px 8px', borderBottom:`1px solid ${BORD}`, fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase' }}>Saved Workflows · {WORKFLOWS.length}</div>
                    {WORKFLOWS.map(w => {
                      const active = w.id === wfId;
                      const statusDot = w.status === 'deployed' ? '#10B981' : w.status === 'scheduled' ? '#F59E0B' : 'rgba(255,255,255,0.4)';
                      return (
                        <button key={w.id} onClick={() => { setWfId(w.id); setSwitcherOpen(false); }} style={{ display:'flex', alignItems:'center', gap:10, width:'100%', padding:'10px 14px', border:'none', borderBottom:`1px solid ${BORD}`, background: active ? `rgba(0,201,167,0.06)` : 'transparent', cursor:'pointer', textAlign:'left' }}
                          onMouseEnter={e => { if (!active) e.currentTarget.style.background='rgba(255,255,255,0.03)'; }}
                          onMouseLeave={e => { if (!active) e.currentTarget.style.background='transparent'; }}>
                          <div style={{ width:8, height:8, borderRadius:'50%', background:statusDot, boxShadow:`0 0 6px ${statusDot}`, flexShrink:0 }}/>
                          <div style={{ flex:1, minWidth:0 }}>
                            <div style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:13, fontWeight:700, color:active?ACCENT:'#fff', letterSpacing:'-0.01em', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{w.name}</div>
                            <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.4)', marginTop:2, letterSpacing:'0.1em' }}>{w.status} · {w.nodes.length} atoms · {w.lastRun} · {w.runs} runs · {w.successRate}%</div>
                          </div>
                          {active && <Check size={13} color={ACCENT} style={{flexShrink:0}}/>}
                        </button>
                      );
                    })}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, flexWrap:'wrap' }}>
            {/* Error-demo toggle */}
            <label style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'6px 10px', borderRadius:9, background:errorMode ? 'rgba(239,68,68,0.08)' : SURF, border:`1px solid ${errorMode ? 'rgba(239,68,68,0.35)' : BORD}`, cursor:'pointer', transition:'all 0.2s', userSelect:'none' }}>
              <div style={{ position:'relative', width:28, height:14, borderRadius:999, background:errorMode ? '#ef4444' : 'rgba(255,255,255,0.18)', transition:'background 0.2s' }}>
                <div style={{ position:'absolute', top:2, left:errorMode ? 16 : 2, width:10, height:10, borderRadius:'50%', background:'#fff', transition:'left 0.2s', boxShadow:'0 1px 3px rgba(0,0,0,0.4)' }}/>
                <input type="checkbox" checked={errorMode} onChange={e => setErrorMode(e.target.checked)} style={{ position:'absolute', inset:0, opacity:0, cursor:'pointer' }}/>
              </div>
              <CircleAlert size={11} color={errorMode ? '#ef4444' : 'rgba(255,255,255,0.4)'}/>
              <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:10, letterSpacing:'0.18em', color:errorMode ? '#fca5a5' : 'rgba(255,255,255,0.6)', textTransform:'uppercase', fontWeight:600 }}>Simulate failure</span>
            </label>

            {/* Status chip */}
            <div style={{ display:'inline-flex', alignItems:'center', gap:7, padding:'7px 12px', borderRadius:999, background:SURF, border:`1px solid ${BORD}` }}>
              <span style={{ width:7, height:7, borderRadius:'50%', background:statusColor, boxShadow:runState.mode!=='idle'?`0 0 8px ${statusColor}`:'none', animation:runState.mode==='running'?'pulse-glow 1.4s ease-in-out infinite':'none' }}/>
              <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:10, letterSpacing:'0.2em', color:'rgba(255,255,255,0.78)', textTransform:'uppercase' }}>{statusLabel}</span>
            </div>
            <button onClick={resetRun} style={{ padding:'8px 14px', borderRadius:9, border:`1px solid ${BORD}`, background:SURF, color:'rgba(255,255,255,0.8)', fontFamily:'Bricolage Grotesque, sans-serif', fontWeight:600, fontSize:12, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:6 }}>
              <Save size={12}/> Save
            </button>
            <button onClick={testRun} disabled={runState.mode==='running'}
        ...
        {/* Main 3-column grid */}
        <div className="builder-grid" style={{ display:'grid', gridTemplateColumns:'220px 1fr 300px', gap:14, height:'calc(100vh - 175px)', minHeight:620 }}>
          {/* ─── TOOLBOX ─── */}
          <div style={{ borderRadius:14, border:`1px solid ${BORD}`, background:'rgba(10,16,28,0.7)', padding:16, overflow:'auto' }}>
            <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.24em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', marginBottom:12 }}>Atom Toolbox</div>
            <div style={{ fontFamily:'Manrope, sans-serif', fontSize:10, color:'rgba(255,255,255,0.4)', marginBottom:8, lineHeight:1.4 }}>Drag atoms onto the canvas to build.</div>
            <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
              {Object.entries(ATOM_TYPES).map(([key, t]) => (
                <div key={key}
                  draggable
                  onDragStart={e => {
                    e.dataTransfer.setData('application/atom-type', key);
                    e.dataTransfer.effectAllowed = 'copy';
                  }}
                  style={{
                    display:'flex', alignItems:'center', gap:10,
                    padding:'9px 10px', borderRadius:9,
                    background:`linear-gradient(135deg, ${t.color}0e, transparent)`,
                    border:`1px solid ${t.color}2a`,
                    cursor:'grab', transition:'all 0.2s',
                    userSelect:'none',
                  }}
                  onMouseEnter={e=>{ e.currentTarget.style.background=`linear-gradient(135deg, ${t.color}1e, transparent)`; e.currentTarget.style.borderColor=t.color+'55'; e.currentTarget.style.transform='translateX(2px)'; }}
                  onMouseLeave={e=>{ e.currentTarget.style.background=`linear-gradient(135deg, ${t.color}0e, transparent)`; e.currentTarget.style.borderColor=t.color+'2a'; e.currentTarget.style.transform='translateX(0)'; }}>
                  <AtomIcon color={t.color} size={22}/>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:12, fontWeight:700, color:'#fff', letterSpacing:'-0.01em' }}>{t.label}</div>
                    <div style={{ fontFamily:'Manrope, sans-serif', fontSize:10, color:'rgba(255,255,255,0.45)', lineHeight:1.3, marginTop:1 }}>{t.desc}</div>
                  </div>
                </div>
              ))}
            </div>

            {/* Saved workflows mini-list */}
            <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.24em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', margin:'20px 0 10px' }}>Your Workflows</div>
            <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
              {WORKFLOWS.map(w => {
                const active = w.id === wfId;
                const statusDot = w.status === 'deployed' ? '#10B981' : w.status === 'scheduled' ? '#F59E0B' : 'rgba(255,255,255,0.4)';
                return (
                  <button key={w.id} onClick={() => setWfId(w.id)} style={{
                    display:'flex', alignItems:'center', gap:7, padding:'7px 10px', borderRadius:8,
                    background:active ? 'rgba(0,201,167,0.1)' : SURF,
                    border:`1px solid ${active ? ACCENT+'55' : BORD}`,
                    ...
                    <span style={{ flex:1, minWidth:0, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', fontSize:11.5, fontWeight:active ? 700 : 500 }}>{w.name.replace('→','→')}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div
            ...
            style={{ position:'relative', borderRadius:14, border:`1px solid ${dropPreview ? ACCENT+'66' : BORD}`, background:'rgba(6,10,18,0.6)', overflow:'hidden', transition:'border-color 0.2s' }}>
            {/* Dot grid */}
            <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(circle, rgba(255,255,255,0.05) 1px, transparent 1px)', backgroundSize:'22px 22px', opacity:0.85, pointerEvents:'none' }}/>
            {/* Major lines, very faint */}
            <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(90deg, rgba(255,255,255,0.025) 1px, transparent 1px), linear-gradient(rgba(255,255,255,0.025) 1px, transparent 1px)', backgroundSize:'110px 110px', pointerEvents:'none' }}/>
            {/* Canvas header */}
            <div style={{ position:'absolute', top:14, left:16, zIndex:20, display:'flex', alignItems:'center', gap:8, padding:'5px 11px', borderRadius:999, background:'rgba(7,13,22,0.85)', border:`1px solid ${BORD}`, backdropFilter:'blur(6px)' }}>
              <Activity size={11} color={statusColor}/>
              <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.2em', color:'rgba(255,255,255,0.65)', textTransform:'uppercase' }}>Canvas · {nodes.length} atoms · {edges.length} bonds</span>
            </div>

            {/* Empty-state hint while dragging over blank canvas */}
            {dropPreview && (
              <div style={{ position:'absolute', top:dropPreview.y, left:dropPreview.x, width:188, height:68, borderRadius:12, border:`2px dashed ${ACCENT}`, background:`rgba(0,201,167,0.08)`, pointerEvents:'none', zIndex:4, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:ACCENT, textTransform:'uppercase' }}>Drop to place</span>
              </div>
            )}
            ...
            <svg style={{ position:'absolute', inset:0, width:'100%', height:'100%', pointerEvents:'none', zIndex:2 }}>
              {edges.map((e, i) => {
                ...
                return (
                  <Bond key={i} from={fromNode} to={toNode} label={e.label}
                        running={isEdgeActive(e)}
                        success={isEdgeComplete(e) && !isEdgeActive(e) && !isEdgeError(e)}
                        error={isEdgeError(e)}/>
                );
              })}
            </svg>
            ...
            <div style={{ position:'relative', zIndex:3, width:'100%', height:'100%' }}>
              {nodes.map(n => (
                <BuilderNode key={n.id} node={n}
                  ...
                  running={runState.active === n.id}
                  success={runState.completed.includes(n.id)}
                  error={runState.mode === 'failed' && runState.failedAt === n.id}
                  onClick={() => { if (!dragStateRef.current.moved) setSelectedId(n.id); }}
                  onMouseDown={handleNodeMouseDown(n.id)}/>
              ))}
            </div>

            {/* Canvas footer */}
            <div style={{ position:'absolute', bottom:14, right:16, zIndex:20, display:'flex', alignItems:'center', gap:7, padding:'6px 10px', borderRadius:999, background:'rgba(7,13,22,0.9)', border:`1px solid ${BORD}` }}>
              <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.4)', letterSpacing:'0.15em' }}>100%</span>
            </div>
          </div>

          {/* ─── INSPECTOR ─── */}
          <div style={{ borderRadius:14, border:`1px solid ${BORD}`, background:'rgba(10,16,28,0.7)', overflow:'auto' }}>
            {selected && (() => {
              const type = ATOM_TYPES[selected.type];
              const TypeIcon = type.Icon;
              const tabs = ['Configure','Logs','History'];
              return (
                <div>
                  {/* Header */}
                  <div style={{ padding:'18px 18px 14px', borderBottom:`1px solid ${BORD}`, background:`linear-gradient(150deg, ${type.color}14, transparent)` }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <AtomIcon color={type.color} size={36}/>
                      <div>
                        <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                          <TypeIcon size={10} color={type.color}/>
                          <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.24em', color:type.color, textTransform:'uppercase', fontWeight:600 }}>{type.label}</span>
                        </div>
                        <div style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:15, fontWeight:700, color:'#fff', marginTop:2, letterSpacing:'-0.01em' }}>{selected.label}</div>
                        <div style={{ fontFamily:'Manrope, sans-serif', fontSize:11, color:'rgba(255,255,255,0.5)', marginTop:1 }}>{selected.subtitle}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display:'flex', borderBottom:`1px solid ${BORD}` }}>
                    {tabs.map((t) => (
                      <button key={t} onClick={() => setInspectorTab(t)} style={{ flex:1, padding:'10px 10px', textAlign:'center', fontFamily:'Bricolage Grotesque, sans-serif', fontSize:11, fontWeight:700, color:inspectorTab===t?'#fff':'rgba(255,255,255,0.4)', borderBottom:inspectorTab===t?`2px solid ${type.color}`:'2px solid transparent', cursor:'pointer', border:'none', borderTop:'none', borderLeft:'none', borderRight:'none', background:'transparent', transition:'color 0.2s' }}>{t}</button>
                    ))}
                  </div>
                  ...
                  {inspectorTab === 'Configure' && (
                    <div style={{ padding:'16px 18px' }}>
                      <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', marginBottom:8 }}>Properties</div>
                      {[
                        ['Label', selected.label],
                        ['Source', selected.subtitle],
                        selected.type==='condition' ? ['Expression', 'amount > 10000'] : null,
                        selected.type==='action'    ? ['Retry policy', 'exponential · 3 tries'] : null,
                        selected.type==='trigger'   ? ['Filter', 'Stage = Closed Won'] : null,
                      ].filter(Boolean).map(([k, v]) => (
                        <div key={k} style={{ marginBottom:10 }}>
                          <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.42)', letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>{k}</div>
                          <div style={{ padding:'8px 10px', borderRadius:7, background:SURF, border:`1px solid ${BORD}`, fontFamily:'JetBrains Mono, monospace', fontSize:11, color:'rgba(255,255,255,0.85)' }}>{v}</div>
                        </div>
                      ))}

                      <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', margin:'18px 0 8px' }}>Connections</div>
                      {edges.filter(e => e.from === selected.id || e.to === selected.id).map((e, i) => {
                        ...
                        const otherType = ATOM_TYPES[other.type];
                        return (
                          <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 10px', borderRadius:7, background:SURF, border:`1px solid ${BORD}`, marginBottom:6 }}>
                            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.4)', letterSpacing:'0.12em', minWidth:24 }}>{isOut?'→':'←'}</span>
                            <AtomIcon color={otherType.color} size={16}/>
                            <span style={{ fontFamily:'Manrope, sans-serif', fontSize:11, color:'rgba(255,255,255,0.8)', flex:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{other.label}</span>
                            {e.label && <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8, letterSpacing:'0.14em', color:otherType.color, textTransform:'uppercase', padding:'1px 6px', borderRadius:999, border:`1px solid ${otherType.color}55` }}>{e.label}</span>}
                          </div>
                        );
                      })}
                      {edges.filter(e => e.from === selected.id || e.to === selected.id).length === 0 && (
                        <div style={{ padding:'8px 10px', borderRadius:7, background:SURF, border:`1px dashed ${BORD}`, fontFamily:'Manrope, sans-serif', fontSize:11, color:'rgba(255,255,255,0.4)', fontStyle:'italic' }}>Not connected yet.</div>
                      )}
                    </div>
                  )}
                  ...
                  {inspectorTab === 'Logs' && (
                    <div style={{ padding:'16px 18px' }}>
                      <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', marginBottom:10, display:'flex', alignItems:'center', gap:6 }}>
                        <span style={{ width:5, height:5, borderRadius:'50%', background:statusColor, boxShadow:runState.mode!=='idle'?`0 0 5px ${statusColor}`:'none' }}/>
                        {runState.mode === 'idle' ? 'No active run' : 'Live run'}
                      </div>
                      {runState.mode === 'idle' ? (
                        <div style={{ padding:'14px', borderRadius:8, background:SURF, border:`1px dashed ${BORD}`, fontFamily:'Manrope, sans-serif', fontSize:12, color:'rgba(255,255,255,0.45)', textAlign:'center' }}>
                          Press <strong style={{ color:ACCENT }}>Test Run</strong> to see live execution logs here.
                        </div>
                      ) : (
                        <div style={{ padding:'10px 12px', borderRadius:8, background:runState.mode==='failed' ? 'rgba(239,68,68,0.06)' : 'rgba(0,201,167,0.06)', border:`1px solid ${runState.mode==='failed' ? 'rgba(239,68,68,0.2)' : 'rgba(0,201,167,0.2)'}`, fontFamily:'JetBrains Mono, monospace', fontSize:10.5, color:'rgba(255,255,255,0.78)', lineHeight:1.75 }}>
                          {runState.completed.map((id, idx) => (
                            <div key={idx} style={{ display:'flex', gap:8 }}>
                              <CircleCheck size={11} color="#10B981" style={{ flexShrink:0, marginTop:3 }}/>
                              <span><span style={{ color:'rgba(255,255,255,0.42)' }}>{String(idx+1).padStart(2,'0')}</span> {nodeById[id]?.label}</span>
                            </div>
                          ))}
                          {runState.active && (
                            <div style={{ display:'flex', gap:8, color:'#00C9A7' }}>
                              <div style={{ width:11, height:11, borderRadius:'50%', border:'2px solid #00C9A7', borderTopColor:'transparent', animation:'orbit-spin 0.8s linear infinite', flexShrink:0, marginTop:3 }}/>
                              <span><span style={{ color:'rgba(255,255,255,0.42)' }}>{String(runState.completed.length+1).padStart(2,'0')}</span> {nodeById[runState.active]?.label} <span style={{ opacity:0.6 }}>…running</span></span>
                            </div>
                          )}
                          {runState.mode==='success' && (
                            <div style={{ color:'#10B981', marginTop:6, fontWeight:700, display:'flex', alignItems:'center', gap:6 }}>
                              <CircleCheck size={13}/> Run complete · {runState.completed.length} atoms fired
                            </div>
                          )}
                          {runState.mode==='failed' && (
                            <>
                              <div style={{ display:'flex', gap:8, color:'#ef4444' }}>
                                <X size={11} color="#ef4444" strokeWidth={3} style={{ flexShrink:0, marginTop:3 }}/>
                                <span><span style={{ color:'rgba(255,255,255,0.42)' }}>{String(runState.completed.length+1).padStart(2,'0')}</span> {nodeById[runState.failedAt]?.label} <span style={{ opacity:0.8 }}>…failed</span></span>
                              </div>
                              <div style={{ color:'#ef4444', marginTop:6, fontWeight:700, display:'flex', alignItems:'center', gap:6 }}>
                                <CircleAlert size={13}/> Run halted at "{nodeById[runState.failedAt]?.label}"
                              </div>
                              <div style={{ marginTop:6, padding:'8px 10px', borderRadius:6, background:'rgba(239,68,68,0.1)', border:'1px solid rgba(239,68,68,0.25)', color:'#fca5a5', fontSize:10 }}>
                                Error: upstream service returned 429 (rate limit). Retry policy: exponential backoff · 3 tries remaining.
                              </div>
                            </>
                          )}
                        </div>
                  ...
                  {inspectorTab === 'History' && (
                    <div style={{ padding:'16px 18px' }}>
                      <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, letterSpacing:'0.22em', color:'rgba(255,255,255,0.4)', textTransform:'uppercase', marginBottom:4 }}>Recent Runs</div>
                      <div style={{ fontFamily:'Manrope, sans-serif', fontSize:10, color:'rgba(255,255,255,0.4)', marginBottom:12 }}>{sourceWf.runs.toLocaleString()} total · {sourceWf.successRate}% success rate</div>
                      {sourceWf.history.map(h => {
                        const col = h.status === 'success' ? '#10B981' : '#ef4444';
                        return (
                          <div key={h.id} style={{ display:'flex', gap:10, padding:'10px 0', borderBottom:`1px solid ${BORD}` }}>
                            <div style={{ width:6, height:6, borderRadius:'50%', background:col, marginTop:6, flexShrink:0, boxShadow:`0 0 5px ${col}` }}/>
                            <div style={{ flex:1, minWidth:0 }}>
                              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'baseline', gap:6 }}>
                                <span style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:12, fontWeight:700, color:'#fff', textTransform:'capitalize' }}>{h.status}</span>
                                <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.4)' }}>{h.time}</span>
                              </div>
                              <div style={{ fontFamily:'Manrope, sans-serif', fontSize:11, color:'rgba(255,255,255,0.6)', marginTop:2 }}>{h.note}</div>
                              <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9, color:'rgba(255,255,255,0.38)', marginTop:3, letterSpacing:'0.1em' }}>duration · {h.duration}</div>
                            </div>
                          </div>
                        );
                      })}
        ...
        <div style={{ padding:28, borderRadius:18, border:`1px solid ${agent.color}40`, background:`linear-gradient(160deg, ${agent.color}12, rgba(255,255,255,0.03))` }}>
          ...
          {agent.id === 'pulse' && (
            <div style={{ marginTop:8 }}>
              <PrimaryButton onClick={onOpenBuilder} icon={Workflow}>Open Workflow Builder</PrimaryButton>
            </div>
          )}
        </div>
...
export default function AscentraPlatform() {
  ...
  const renderPage = () => {
    ...
    if (page.view === 'agent' && pageAgent) return <AgentWorkspacePage agent={pageAgent} onBack={goHome} onDocs={() => openDocsPage(pageAgent)} onOpenBuilder={openBuilderPage}/>;
    if (page.view === 'builder') return <BuilderPage onBack={() => setPage({ view:'agent', agentId:'pulse' })}/>;
  ...
  return (
    <div style={{ minHeight:'100vh', background:BG, color:'#fff', fontFamily:'Manrope, sans-serif', overflow:'hidden' }}>
      <style>{`
        ...
        /* Pulse orchestration halo — sequenced chain signal */
        @keyframes bond-glow-a { 0%,18%,100%{opacity:.35;stroke-width:1.2} 10%{opacity:1;stroke-width:2.2} }
        @keyframes bond-glow-b { 0%,100%{opacity:.35;stroke-width:1.2} 36%{opacity:.35} 44%{opacity:1;stroke-width:2.2} 52%{opacity:.35} }
        @keyframes bond-glow-c { 0%,100%{opacity:.35;stroke-width:1.2} 70%{opacity:.35} 78%{opacity:1;stroke-width:2.2} 86%{opacity:.35} }
        @keyframes sat-breathe-a { 0%,14%,100%{transform:scale(1);filter:brightness(1)} 10%{transform:scale(1.18);filter:brightness(1.4)} }
        @keyframes sat-breathe-b { 0%,100%{transform:scale(1);filter:brightness(1)} 40%{transform:scale(1);filter:brightness(1)} 46%{transform:scale(1.18);filter:brightness(1.4)} 52%{transform:scale(1);filter:brightness(1)} }
        @keyframes sat-breathe-c { 0%,100%{transform:scale(1);filter:brightness(1)} 74%{transform:scale(1);filter:brightness(1)} 80%{transform:scale(1.18);filter:brightness(1.4)} 86%{transform:scale(1);filter:brightness(1)} }

        /* Automation Atom Grid Builder */
        @keyframes edge-flow      { from{stroke-dashoffset:24} to{stroke-dashoffset:0} }
        @keyframes signal-travel  { from{offset-distance:0%;opacity:0} 10%{opacity:1} 90%{opacity:1} to{offset-distance:100%;opacity:0} }
        @keyframes node-run-ring  { 0%{box-shadow:0 0 0 0 rgba(0,201,167,0.55)} 70%{box-shadow:0 0 0 12px rgba(0,201,167,0)} 100%{box-shadow:0 0 0 0 rgba(0,201,167,0)} }
        @keyframes node-breathe   { 0%,100%{filter:brightness(1)} 50%{filter:brightness(1.15)} }
        @keyframes dash-march     { from{stroke-dashoffset:0} to{stroke-dashoffset:-20} }
        @keyframes error-shake    { 0%,100%{transform:translateX(0)} 20%{transform:translateX(-3px)} 40%{transform:translateX(3px)} 60%{transform:translateX(-2px)} 80%{transform:translateX(2px)} }
